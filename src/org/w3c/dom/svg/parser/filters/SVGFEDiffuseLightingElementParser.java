...
import org.w3c.dom.svg.filters.SVGFEDiffuseLightingElement;
...
public class SVGFEDiffuseLightingElementParser implements ElementParser<SVGFEDiffuseLightingElement> {

	@Override
	public SVGFEDiffuseLightingElement readElement(Element element, ParsingState parsingState) {
		...
		String diffuseConstantStr = ElementParser.readOrDefault(element, Attributes.DIFFUSE_CONSTANT, "1");
		SVGAnimatedNumber diffuseConstant = new SVGAnimatedNumber.Implementation(Float.parseFloat(diffuseConstantStr), Float.parseFloat(diffuseConstantStr));
	...
	@Override
	public Element writeElement(SVGFEDiffuseLightingElement element, ElementFactory factory) {
		...
		attributes.put(Attributes.DIFFUSE_CONSTANT, Float.toString(element.getDiffuseConstant().getBaseValue()));
		...
		return factory.createElement(Tags.FE_DIFFUSELIGHTING, attributes);
	}
